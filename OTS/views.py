...
def calculateTestResult(request):
    ...
    # Check if qid_list is not empty to avoid division by zero
    if qid_list:
        for n in qid_list:
        ...
        # Handle the case where no questions were answered
        points = 0
    ...
    candidate.test_attemted += 1  
    candidate.points = (candidate.points * (candidate.test_attemted - 1) + points) / candidate.test_attemted
    candidate.save()
...
def TestResultHistory(request):
    ...
    candidate = Candidate.objects.get(username=request.session['username'])    
    results = Result.objects.filter(username=candidate).order_by('-date')
